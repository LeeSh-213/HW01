...
int main() {
	...
	// 배열의 합계를 계산하는 코드
	for (int i = 0; i < SIZE; ++i) // 0부터 4까지 반복하여 배열의 각 요소를 합산합니다.
	{
		u += numbers[i]; // 현재까지의 합계를 저장하는 변수 u에 배열의 각 요소를 더합니다.
	}

	avg = static_cast<double>(u) / SIZE; // 합계를 SIZE로 나누어 평균을 계산합니다.

	cout << "배열의 합계를 계산합니다. : " << u << endl; // 합계는 정수로 출력됩니다.
	cout << "배열의 평균을 계산합니다. : " << avg << endl; // 평균은 소수점 이하까지 출력됩니다.
